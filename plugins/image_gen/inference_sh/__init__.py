...
implementation — one API key, seven model families:
    ...
    p-image              ~5s     Fastest and cheapest ($0.0001/image)
    flux-dev-lora        ~15s    FLUX with LoRA style support
    seedream-4.5         ~30s    4K cinematic, text-in-image
    gpt-image-2          ~30s    OpenAI, editing + inpainting
    gemini-image-3-pro   ~30s    Google, high fidelity
    grok-imagine         ~15s    xAI, fast creative
    reve                 ~20s    Natural language editing
...
5. :data:`DEFAULT_MODEL` — ``flux-dev-lora``
...
_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "reve": {
        "display": "Reve",
        "speed": "~20s",
        "strengths": "Natural language image editing and generation",
        "app": "reve/image",
    },
}

DEFAULT_MODEL = "flux-dev-lora"
...
class InferenceShImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "badge": "paid",
            "tag": (
                "FLUX, Seedream 4.5, GPT-Image-2, Gemini, Grok Imagine, Reve, P-Image "
                "— one API key for all models (inference.sh)"
            ),
            "env_vars": [
                {
